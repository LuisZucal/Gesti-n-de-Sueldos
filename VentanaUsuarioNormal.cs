    ...
    public partial class VentanaUsuarioNormal : Form
    {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            if (sueldoCalculado)
            {
                ...
                string sueldoBruto = lblSueldoBruto.Text;
                string descuentoAFP = lblDescuentoAFP.Text;
                string descuentoSalud = lblDescuentoSalud.Text;
                string sueldoLiquido = lblSueldoLiquido.Text;

                // Crear una cadena de texto con la información a guardar
                string informacionAGuardar = $"{sueldoBruto} | {descuentoAFP} | {descuentoSalud} | {sueldoLiquido}";

                // Especifica la ubicación del archivo donde deseas guardar la información
                string filePath = "informacion_sueldo.txt";

                // Escribe la información en el archivo de texto
                using (StreamWriter writer = File.CreateText(filePath))
                {
                    writer.Write(informacionAGuardar);
                }

                // Muestra un mensaje de éxito después de guardar.
                MessageBox.Show("La información se ha guardado con éxito en el archivo 'informacion_sueldo.txt'.", "Guardado Exitoso", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
